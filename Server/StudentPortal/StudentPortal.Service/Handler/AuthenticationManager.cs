...
namespace StudentPortal.Service.Handler
{
    public class AuthenticationManager : IAuthenticationManager
    {
        ...
        public string BuildToken(User user)
        {
            ...
            var claims = new List<Claim> {
                    ...
                    new Claim(JwtClaims.Name, user.UserName?? string.Empty),
                    new Claim(JwtClaims.Email, user.Email?? string.Empty),
                    new Claim(JwtClaims.ExpiresDate, expDate.ToString()),
                    new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()),
                    new Claim(Common.Constant.JwtClaims.AccessRight, user.UserTypeId.ToString() ?? string.Empty)
                };
